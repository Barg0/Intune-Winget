...
foreach ($app in $wingetApps) {
    ...
    Write-Log "Checking if $appName is installed..." -Tag "Check"
    $installedOutput = & .\winget.exe list -e --id $appID --accept-source-agreements
    $exitCode = $LASTEXITCODE

    if ($exitCode -eq -1978335212 -and $installedOutput -match 'No installed package') {
        Write-Log "$appName is NOT installed." -Tag "Success"
    }
    elseif ($exitCode -ne 0) {
        Write-Log "Winget failed while checking $appName (exit code: $exitCode)" -Tag "Error"
    }
    else {
...
if ($appDetected) {
    Complete-Script -ExitCode 1  # Trigger remediation
} else {
