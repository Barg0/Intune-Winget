...
# ---------------------------[ Winget app detection ]---------------------------

try {
    $wingetPath = Get-WingetPath

    # First attempt: by Winget ID
    $installedOutput = & $wingetPath list -e --id $wingetAppID --accept-source-agreements
    $wingetExitCode = $LASTEXITCODE

    if ($wingetExitCode -eq 0 -and $installedOutput) {
        Write-Log "App detected (by ID) - $applicationName IS installed." -Tag "Success"
        Complete-Script -ExitCode 0
    }
    elseif ($wingetExitCode -eq -1978335212 -or $installedOutput -match 'No installed package found matching input criteria.') {
        Write-Log "Detection by ID failed (exit code $wingetExitCode). Trying fallback detection by Name..." -Tag "Error"

        # Fallback: by Name
        $fallbackOutput = & $wingetPath list --name "$applicationName" --accept-source-agreements
        $fallbackExitCode = $LASTEXITCODE

        if ($fallbackExitCode -eq 0 -and $fallbackOutput -match [regex]::Escape($applicationName)) {
            Write-Log "App detected (by Name) - $applicationName IS installed." -Tag "Success"
            Complete-Script -ExitCode 0
        }
        else {
        ...
        Write-Log "Winget detection failed with unexpected exit code: $wingetExitCode" -Tag "Error"
        Complete-Script -ExitCode 1
    }
}
catch {
